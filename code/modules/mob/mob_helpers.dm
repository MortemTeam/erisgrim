...
/proc/slur(phrase)
	...
	while(counter>=1)
		...
		switch(rand(1,15))
			if(1,2,3,4,5,6,7)	    newletter="[lowertext(newletter)]"
			if(8,9,10,11,12,13,14)	newletter="[uppertext(newletter)]"
			if(15)	newletter+="'"
...
/proc/stutter(n)
	...
	while(p <= n)//while P, which starts at 1 is less or equal to N which is the length.
		...
		if (prob(80) && (lowertext(n_letter) in LIST_OF_CONSONANT|LIST_OF_CONSONANT))
			if (prob(10))
